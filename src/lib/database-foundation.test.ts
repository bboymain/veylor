...
describe("four-table database foundation", () => {
  test("Phase 7 creates brands, products, and alternatives linked to searches", async () => {
    const sql = await readMigration(phase7MigrationPath);
  ...
  test("Stage 8 is additive and preserves legacy search data", async () => {
    const sql = (await readMigration(stage8MigrationPath)).toLowerCase();
  ...
  test("Stage 8 validates relationships and keeps every table server-only", async () => {
    const sql = await readMigration(stage8MigrationPath);
  ...
  test("mutable tables receive updated_at triggers", async () => {
    const sql = await readMigration(stage8MigrationPath);
