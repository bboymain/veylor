...
describe("benchmark runner", () => {
  test("integrated dry-run uses the injectable provider and performs no database writes", async () => {
    const manifest = loadManifest(EXAMPLE_MANIFEST_PATH);
    ...
    const result = await runBenchmarkIntegrated(
      manifest,
      provider,
      { provider: "mock", model: "fixture-v1" },
      {
        ensureCases: unexpected,
        getRunStatus: unexpected,
        startRun: unexpected,
    ...
    expect(result.run.averageOverallScore).toBe(1);
    expect(result.persistence.writesPerformed).toBe(0);
  });
