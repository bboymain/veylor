...
export async function runBenchmarkIntegrated(
  manifest: BenchmarkManifest,
  provider: BenchmarkProvider,
  metadata: { provider: string; model: string },
  persistence: BenchmarkPersistence,
  options: { dryRun?: boolean; idempotencyKey?: string } = {},
): Promise<{ run: BenchmarkRunResult; persistence: PersistenceOutcome }> {
  const run = await runBenchmark(manifest, provider);
  const persistenceOutcome = await persistBenchmarkRun(manifest, metadata, run, persistence, {
